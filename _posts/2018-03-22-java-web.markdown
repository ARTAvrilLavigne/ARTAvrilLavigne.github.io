...
<p> String name：该Cookie的名称。Cookie一旦创建，名称便不可更改。 
    ...
    int version：该Cookie使&gt;用的版本号。0表示遵循Netscape的Cookie规范，1表示遵循W3C的RFC 2109规范。</p>
...
<blockquote>
<p>void setAttribute(String attribute, Object value)：设置Session属性。value参数可以为任何Java Object,通常为Java Bean。value信息不宜过大
    ...
    boolean isNew()：返回该Session是否是&gt;新创建的 void invalidate()：使该Session失效</p>
</blockquote>
...
<p>由于Cookie可以被人为的禁止，必须有其他机制以便在Cookie被禁止时仍然能够把session id传递回服务器。经常被使用的一种技术叫做URL重写，就是把session id直接附加在URL路径的后面，附加方式也有两种： 一种是作为URL路径的附加信息，表现形式为<a href="http://" class="uri">http://</a>...../xxx;jsessionid= ByOK3vjFD75aPnrF7C2HmdnV6QZcEbzWoWiBYEnLerjQ99zWpBng!-145788764 一种是作为查询字符串附加在URL后面，表现形式为<a href="http://" class="uri">http://</a>...../xxx?jsessionid=ByOK3vjFD75aPnrF7C2HmdnV6QZcEbzWoWiBYEnLerjQ99zWpBng!-145788764</p>
